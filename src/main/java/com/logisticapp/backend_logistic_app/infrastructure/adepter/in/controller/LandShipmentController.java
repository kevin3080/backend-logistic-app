...
public class LandShipmentController {
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "404",
                        description = "Related resource not found (Client, Product, Warehouse)",
                        content = @Content)
            })
    @PostMapping
    public ResponseEntity<LandShipment> createLandShipment(@Valid @RequestBody LandShipmentRequest request) {
        ...
        Warehouse warehouse = warehouseRepository
                ...
                .orElseThrow(() ->
                        new ResourceNotFoundException("Warehouse not found with ID: " + request.getWarehouseId()));
    ...
    @ApiResponse(
            ...
            content = {@Content(mediaType = "application/json", schema = @Schema(implementation = LandShipment.class))})
    @GetMapping
    ...
    @PutMapping("/{id}")
    public ResponseEntity<LandShipment> updateLandShipment(
            @PathVariable UUID id, @Valid @RequestBody LandShipmentRequest request) {
        Client client = clientRepository
                .findById(request.getClientId())
                .orElseThrow(() -> new ResourceNotFoundException("Client not found with ID: " + request.getClientId()));
        Product product = productRepository
