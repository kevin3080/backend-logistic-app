...
public class MaritimeShipmentController {
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "422",
                        description = "Unprocessable Entity (e.g., duplicate guide number)",
                        content = @Content),
                @ApiResponse(
                        responseCode = "404",
                        description = "Related resource not found (Client, Product, Port)",
                        content = @Content)
            })
    @PostMapping
    public ResponseEntity<MaritimeShipment> createMaritimeShipment(
            @Valid @RequestBody MaritimeShipmentRequest request) {
        ...
        MaritimeShipment newShipment = new MaritimeShipment(
                ...
                0.0,
                request.getFleetNumber(),
                request.getGuideNumber());
    ...
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Found the shipment",
                        content = {
                            @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = MaritimeShipment.class))
    ...
    public ResponseEntity<MaritimeShipment> getMaritimeShipmentById(@PathVariable UUID id) {
        return getMaritimeShipmentUseCase
                .getShipmentById(id)
    ...
    @ApiResponse(
            ...
            content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = MaritimeShipment.class))
            })
    @GetMapping
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "404",
                        description = "Shipment or related resource not found",
                        content = @Content),
                @ApiResponse(
                        responseCode = "422",
                        description = "Unprocessable Entity (e.g., duplicate guide number)",
    ...
    @PutMapping("/{id}")
    public ResponseEntity<MaritimeShipment> updateMaritimeShipment(
            @PathVariable UUID id, @Valid @RequestBody MaritimeShipmentRequest request) {
        Client client = clientRepository
                .findById(request.getClientId())
                .orElseThrow(() -> new ResourceNotFoundException("Client not found with ID: " + request.getClientId()));
        Product product = productRepository
        ...
        MaritimeShipment updatedShipment = new MaritimeShipment(
                ...
                port,
                request.getShippingCost(),
                0.0, // Discount will be applied by service
                request.getFleetNumber(),
                request.getGuideNumber());
