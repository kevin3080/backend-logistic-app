...
public class MaritimeShipmentService
        implements CreateMaritimeShipmentUseCase,
                GetMaritimeShipmentUseCase,
                UpdateMaritimeShipmentUseCase,
                DeleteMaritimeShipmentUseCase {
    ...
    public MaritimeShipment createShipment(MaritimeShipment shipment) {
        if (saveMaritimeShipmentPort.existsByGuideNumber(shipment.getGuideNumber())) {
            throw new DuplicateGuideNumberException(
                    "Maritime shipment with guide number " + shipment.getGuideNumber() + " already exists.");
        }
    ...
    public MaritimeShipment updateShipment(UUID id, MaritimeShipment updatedShipment) {
        return saveMaritimeShipmentPort
                .findById(id)
                .map(existingShipment -> {
                    ...
                    if (!existingShipment.getGuideNumber().equals(updatedShipment.getGuideNumber())
                        ...
                        throw new DuplicateGuideNumberException("Maritime shipment with guide number "
                                + updatedShipment.getGuideNumber() + " already exists.");
                    }
