...
public class LandShipmentService
        implements CreateLandShipmentUseCase,
                GetLandShipmentUseCase,
                UpdateLandShipmentUseCase,
                DeleteLandShipmentUseCase {
    ...
    public LandShipment createShipment(LandShipment shipment) {
        if (saveLandShipmentPort.existsByGuideNumber(shipment.getGuideNumber())) {
            throw new DuplicateGuideNumberException(
                    "Land shipment with guide number " + shipment.getGuideNumber() + " already exists.");
        }
        ...
        return saveLandShipmentPort
                ...
                .map(existingShipment -> {
                    if (!existingShipment.getGuideNumber().equals(updatedShipment.getGuideNumber())
                        ...
                        throw new DuplicateGuideNumberException("Land shipment with guide number "
